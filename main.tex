...
\bigskip
\begin{abstract}
Monitoring the incidence of new infections during a pandemic is critical for effective public health response. General population prevalence surveys for SARS-CoV-2 provided high-quality data for estimating incidence, yet accurate estimation hinges on understanding the duration individuals remain detectable via RT-PCR testing. This study addresses this need using data from the Coronavirus Infection Survey (CIS), a long-term, longitudinal, general population survey conducted in the UK. Analysing these data presents unique challenges such as double interval censoring, undetected infections, and false negatives. We propose a Bayesian survival analysis approach, estimating a non-parametric discrete-time distribution of survival durations and integrating prior information derived from a complementary study. Our methodology is validated through a simulation study, including its resilience to model misspecification, and then applied to the CIS dataset, producing the first comprehensive estimate of detectable duration in a general population. This results in the first estimate of the full duration distribution in a general population sample, as well as methodology that could be transferred to new contexts.
\end{abstract}

\noindent%
{\it Keywords:}  Survival analysis, false negatives, missclasification, Bayesian inference
\vfill
...
The COVID-19 pandemic caused by the SARS-CoV-2 virus is, to date, estimated to have killed approximately 30 million people globally~\citep{whoCOVIDExcess,economistCOVIDExcess}, stretched healthcare services to the brink of collapse~\citep{fongNHS}, and disrupted societies worldwide.
This pandemic highlighted the critical need for an effective public health response, informed by quantifying both the current and future burden. One such key quantity is the \emph{incidence of infection}, defined as the rate at which new infections occur.

The importance of incidence as an epidemiological quantity arises because it drives important outcomes for health such as hospital admissions and deaths. 
More explicitly, given a number of new SARS-CoV-2 infections on one day, we expect some fraction of this number to develop symptoms, a smaller fraction t
to require hospitalisation, and a still smaller fraction to die.
This means that data from testing based on symptoms, on hospitalisations, and on deaths, can be used to estimate incidence.
Such estimation, however, will be sensitive to often highly uncertain values of the fractions of new infections that develop symptoms, that require hospitalisation, and that die.

Community surveillance of the epidemic based on a random population sample can be used to estimate incidence directly provided individuals in the cohort are tested regularly, although the exact time of infection between a negative and positive test will need to be imputed.
Such community surveillance can be used, without repeated measurements of participants, to estimate prevalence of infection -- \ie the number of individuals currently positive -- in an unbiased and accurate manner.
While prevalence is an important epidemiological quantity in its own right, it is not so directly related to e.g.\ hospitalisations as incidence is.
...
In addition to the relatively uncertain approaches of estimating incidence from outcomes like hospitalisations or measuring it via repeated testing of a cohort, we can also work to infer it from two
relatively accurately measurable quantities: community prevalence; and the
distribution of duration of test positivity after infection. In this work,
we present a method for this task, applied to community data on COVID,
in which incidence of infection is estimated through back-calculation~\citep{brookmeyerBackcalculation}, a methodology which relates observations of a disease outcome (\eg occurence of symptoms) to its incidence through a delay distribution and a convolution equation.
In the context of SARS-CoV-2, this approach allows estimation of incidence based on: a time series of the proportion of the population that have a detectable infection; and the distribution of the duration of infection episodes, \ie the number of days an individual is detectable for following infection.
As SARS-CoV-2 is detected by performing Reverse Transcription Polymerase Chain Reaction (RT-PCR) testing on an appropriate sample (\eg a nasal swab), this duration is the period over which an infected individual would return a positive RT-PCR result.
In the UK, estimates of the proportion of the population that were detectable were available from two large-scale prevalence surveys~\citep{cisMethodsONS,rileyREACT}.
...
The CIS was a unique large-scale longitudinal study of RT-PCR positivity in the general population, performing monthly RT-PCR testing on 3--400,000 individuals for nearly three years. (see \cref{sec:data}).
...
However, the CIS study design, with weekly (for an individual's first month after enrolment) and then approimately four-weekly testing intervals, and its implementation pose several challenges to the estimation.
...
\begin{figure}
  ...
  \caption{%
    ...
    (C) Doubly interval censored episodes, shaded regions indicate the censoring bounding the start between $l_j^{(b)}$ and $r_j^{(b)}$, and the end between $l_j^{(e)}$ and $r_j^{(e)}$.
  }
...
The sensitivity, although not the specificity, of the testing procedure is substantially less than 100\%~\citep{cisMethodsONS}; we refer to clinical sensitivity and specificity throughout this article, incorporating all reasons for misclassified test results, including poor self-swabbing, contamination.
...
\citet{piresIntervalMisclassify} providing a comprehensive review of approaches. However, including false negatives with either doubly interval censored data or missed events has not been addressed. The CIS data requires both.
...
The paper is structured as follows: we start by describing the design of the CIS and the data it collects in \cref{sec:data}, then develop the survival analysis framework in \cref{sec:inference};
...
Additionally, it is prior to both vaccination and the emergence of the alpha variant, which potentially impacted the duration of infection episodes~\citep{hakkiOnset,russellWithinhost}.
Our analysis considers a cohort of $\Ncis = \numprint{437590}$ individuals with at least one test in this period.
...
\section{Duration modelling} \label{sec:modelling}

The target of inference is $\vec{\theta}$, the parameters of the survival function $S_{\vec{\theta}}(t) = \prob(D_j \geq t \mid \vec\theta)$ for random variable $D_j$, representing the number of days on which a positive result would be returned by a RT-PCR testing procedure that has 100\% specificity and sensitivity due to infection episode $j$.
We adopt a Bayesian framework to derive a posterior distribution for $\vec{\theta}$ given the partial information provided by the set of observed vectors $\{ o_j \}$ and accounting for the double interval censoring and undetected episodes.
The composition of $\vec{\theta}$ and appropriate priors for $\theta$ are discussed in \cref{sec:parameters-priors}.
In this section, we explain the data generating process and derive a statistical model for the case where that there are no misclassified test results; in \cref{sec:false-negatives} we generalise the framework to include false negatives.

An important aspect of our approach is that the dimension of the posterior distribution estimated via MCMC does not increase with either the cohort size of number of detected infections.
...
Such an approach would be computationally prohibitive here because of the large number of detected infections and the regulatory requirement for the data to be stored on a Trusted Research Environment, the ONS Secure Research Service (SRS)~\citep{onsSRS}, which cannot handle computationally-intensive methods.
...
For our purposes, the $j$th ($j = 1, \dots, \ntot$ where $\ntot$ is the total number of infection epsidoes) infection episode is the triplet $W_j = (B_j, E_j, i_j)$ where $B_j$ is the beginning of the episode, the first day the individual is detectable; $E_j$ is the end of the episode, the last day the individual is detectable; and $i_j$ is the individual in which the infection occurs.
The episode's duration is a deterministic transformation of these variables, $D_j = E_j - B_j + 1$; and I assume that $B_j$, $D_j$, and $i_j$ are independent.
...
Additionally, we assume that the $D_j$s are independent of each other and $B_j$, and identically distributed.
...
The set of times $i_j$ tests positive at due to $W_j$ is $\posResults(W_j) = \{ t \in \sched_{i_j} \ssep B_j \leq t \leq E_j \}$.
...
If $\posResults(W_j) = \emptyset$, $\min \posResults(W_j) \notin [1, T]$, or either $\negResults^{(b)}$ or $\negResults^{(e)}$ is undefined then the episode is undetected, in which case $O_j = \emptyset$.
...
Define an integer $N_E$ and $\set{E} = \{ \vec{\nu}_1, \dots, \vec{\nu}_{N_E} \}$ as the set of all possible values $O(j)$ can take, excluding $\varnothing$, conditional on the test schedules but no other data; that is, $O(j) \in \set{E}$ if and only if $j$ is a detected infection.
...
By assumption $\prob(i_j = i_k \mid \vec{\theta}) = 1/\Ncis$ for all $j$ and $k$.
...
Additionally, we limit the number of permutations of false negative and true positive tests that we consider to have non-negligible probability.
...
To do so, define a random vector $O'(j)$ with state space $\{\varnothing\} \cup \set{E}'$ which will replace $O(j)$.
As before, $O'(j) = \varnothing$ if episode $j$ is undetected.
...
We simulate from a misspecified model, avoiding the independence assumption made in \cref{sec:modelling} and without assuming that some patterns of test results have negligible probability, as in \cref{sec:false-negatives}.
...
Unlike in the simulation studies, an uninformative prior on $\ntot$ led to implausible estimates of the duration distribution (small values of $r$ in \cref{fig:priors}(A) give estimates with a median survival time of 5 days).
...
This CIS data is unique, and is may serve as a template for population prevalence studies in future pandemics.
Accordingly, our methodological framework, which is generic, will inform these studies.
...
This is a higher proportion than estimated using other studies such as ATACCC (see \cref{imperf-test:fig:cis-estimates}), leading to a longer mean, resulting from the better information resulting from the longer follow-up of the CIS.
Long episodes are unlikely to be infectious, but change the interpretation of positive test results, determine how long population positivity remains high following a peak in infections and, crucially, the longer mean duration implies lower incidence when estimated from known prevalence.
...
In addition, we conducted a sensitivity analysis to the simulation study allowing the rate at which infection episodes to either grow or shrink exponentially; these did not make a substantial difference to the estimates (not shown).
...
Further work could consider the impact of this design on the estimates, for example through a random effect for household.
...
The simulation study assumed non-independence between infections in the same individual, with each individual experiencing at most one infection, a plausible assumption over the short period of time considered~\citep{milneImmunity}.
Despite this, the method we propose recovered the true survival function.
...
Future pandemic plans should consider how to alleviate such issues to enable rapid analysis in future emergencies.
