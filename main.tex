...
This pandemic highlighted the critical need for an effective public health response, informed by key quantities such as the \emph{incidence of infection}, defined as the rate of new infections per calendar time~\citep{freemanPrevalence}, which drives important health outcomes including hospital admissions and deaths. 
...
Our analysis includes the cohort of $\Ncis = \numprint{340246}$ CIS participants with at least one non-void test in this period and whose first ever CIS test is not on the final day; the latter are excluded because they have no chance to test positive in the period considered.
...
For the $j$th infection episode ($j = 1, \dots, \ntot$, where $\ntot$ is the unobserved total number of infection episodes), if the episode is detected, there are up to four important times: the day after the negative test in that individual prior to the first positive test, $l_j^{(b)}$; the day of the first positive test, $r_j^{(b)}$; the day of the last positive test, $l_j^{(e)}$; and the day before the subsequent negative test, $r_j^{(e)}$ (see \cref{fig:challenges}(B)).
...
Here, an individual's true infection status refers to whether their viral load is above the test's limit of detection.
...
The strongly informative prior includes information from a previous study~\citep{blakeThesis} using the Assessment of Transmission and Contagiousness of COVID-19 in Contacts (ATACCC) dataset~\citep{hakkiOnset}, and hence is a form of evidence synthesis.
...
At values above the median, the survival function (blue) decreases more slowly than the ATACCC-based estimate (red), indicating a fatter tail.
...
This work is motivated by the challenge of estimate the duration of SARS-CoV-2 infection episodes from CIS data, a unique and long-running general population prevalence study.
This duration is a key component to estimate incidence of infection and inform mitigation strategies such as isolation.
...
To address the challenges posed by the CIS design, we extended the survival analysis framework in \citet{heiseyModelling} resulting in new methodology to analyze doubly censored data with imperfect test sensitivity and undetected events, without any restrictions on when testing occurs or the occurrence of infections.
These challenges arose because study design did not explicitly consider the need to estimate duration of positivity.
To aid in future design, we have developed a flexible simulation framework that can assist with  more efficient survey designs; for example, embedding an intensively sampled, ATACCC-like study within a CIS-like study.
...
The main modelling assumption is that infection episodes (i) have a uniform prior distribution for their start time and (ii) are independent.
...
Exploring the implications of what is possible here would be an exciting avenue for future work.
For example, if there are simple parametric forms for the force of infection (e.g. exponential) that plausibly reflect reality without greatly increasing computational complexity.
...
Code to reproduce all figures and the simulation study in this paper are available at \url{https://github.com/joshuablake/COVID-duration-paper}.
The raw data from the CIS is stored on the ONS SRS due to regulatory requirements.
