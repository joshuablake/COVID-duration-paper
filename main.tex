...
We consider both a weakly and strongly informative prior for $\vec{\theta}$ (see \cref{sec:details-priors}).
...
\end{align}
A full derivation is in \cref{sec:full-posterior-derivation}.
...
\Cref{sec:derive-prob-undetected} shows that
\begin{align}
  1 - p_u
  &\propto \frac{1}{\Ncis} \sum_{i=1}^{\Ncis}\sum_{b = \min \sched_{i} + 1}^{T_{i}} S_{\vec{\theta}}(\tau_{\sched_{i}}(b) + 1)
\end{align}
...
First, the episode would have been detected considering only the mechanisms in \cref{sec:prob-undetected}.
...
We simulate a dataset of detected episodes that has the same characteristics as that in the CIS (see \cref{sec:simulation-data} for details), considering four scenarios for the test sensitivity.
