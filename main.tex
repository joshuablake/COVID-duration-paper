...
Furthermore, they did not estimate the duration distribution in the published estimate.
...
Specifically, for individuals without a positive test, we do not know if they were infected but undetected, or if they were never infected (compare the two situations in \cref{fig:challenges}(A)).
...
\begin{figure}
  ...
  \caption{%
    ...
    (C) Doubly interval censored episodes, shaded regions indicate the censoring bounding the start between $l_j^{(b)}$ and $r_j^{(b)}$, and the end between $l_j^{(e)}$ and $r_j^{(e)}$ (defined later).
  }
...
\section{A Bayesian approach to estimation} \label{sec:modelling}
...
For our purposes, the $j$th ($j = 1, \dots, \ntot$, where $\ntot$ is the total number of infection episodes and unobservable) infection episode is the triplet $W_j = (B_j, E_j, i_j)$ where $B_j$ is the beginning of the episode, the first day the individual is detectable; $E_j$ is the end of the episode, the last day the individual is detectable; and $i_j$ is the individual in which the $j$th infection occurs.
...
\caption{
    ...
    Adjusted to days since first positive by assuming a two day difference, as reported in the paper, with 95\% binomial confidence interval using the Clopper--Pearson method~\citep{clopperUse}.
    ATACCC-based is the median and 95\%CrI as used for the informative prior (see \cref{sec:ataccc-prior}); the analysis does not extend to 88 days.
