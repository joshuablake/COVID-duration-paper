...
For our purposes, an infection episode is the triplet $W_j = (B_j, E_j, i_j)$ where $B_j$ is the beginning of the episode, the first day the individual is detectable; $E_j$ is the end of the episode, the last day the individual is detectable; and $i_j$ is the individual in which the infection occurs.
Infection episode $j$ is \emph{detected} if there is at least one positive test for $i_j$ in the interval $[B_j, E_j]$ and $j$ satisfies the inclusion criteria just described.
...
Therefore, all information from the observations of a detected infection episode $j$ is fully contained in the vector $o_j = [l_j^{(b)}, r_j^{(b)}, l_j^{(e)}, r_j^{(e)}, i_j]^{\mathsf{T}}$ where $i_j$ is the individual in which episode $j$ occurs.
...
We assume the observation $o_j = [l_j^{(b)}, r_j^{(b)}, l_j^{(e)}, r_j^{(e)}, i_j]^{\mathsf{T}}$ is a realisation of the discrete random variable $O_j$, taking values in $\set{O} = \{ \varnothing \} \cup \set{E}$ where $\set{E} = \{ \vec{\nu}_1, \dots, \vec{\nu}_{N_E} \}$ is the set of all $N_E$ (a priori) possible unique observations of detected episodes and $o_j = \varnothing$ if and only if $o_j$ is undetected.
Therefore, $\vec{\nu}_k = [l^{(b)}_k, r^{(b)}_k, l^{(e)}_k, r^{(e)}_k, i_k]^{\mathsf{T}}$ for $k = 1, \dots, N_E$.
The state space is illustrated in \cref{perf-test:fig:partitionSpace}.
% Infection episodes are indexed by $j = 1, \dots, \ntot$.
% If $j$ is a detected episode, then $o_j = [l_j^{(b)}, r_j^{(b)}, l_j^{(e)}, r_j^{(e)}, i_j]^{\mathsf{T}}$, as before. 
% If $j$ is undetected, then define $o_j = \varnothing$.
...
Represent the observations such that for any $\vec{\nu}_k' \in \set{E}'$, $\vec{\nu}_k' = [\vec{\nu}_{k}, \vec{y}_k]^{\mathsf{T}}$, where $\vec{y}_k \in \{ 0, 1 \}^{m_k}$ with $y_{k,l}$ being the test result at time $t_{k,l}$.
...
If $j$ is detected then $O'_j = [O_j, Y_j]^{\mathsf{T}} \in \set{E}'$; otherwise, $O'_j = \varnothing$.
