...
This study tested individuals at regular intervals for a long period of time (see \autoref{sec:data}).
...
Monthly testing is longer than many infection episodes; therefore, an infection episode could begin and end between tests (see \autoref{fig:challanges}(A)).
...
That is, both $B_j$ and $E_j$ are only known up to an interval (see \autoref{fig:challenges}(B)).
...
\citet{sunAnalysis,bogaertsSurvival} review methods for doubly interval censored data.
...
General theoretical frameworks, which include this case, have been developed~\citep{turnbullEmpirical,dempsterMaximum}.
Yet only the special case where the terminating event is either uncensored or right censored has been applied in practice~\citep[e.g.][]{sunEmpirical,bacchettiNonparametric,shenNonparametric}.
...
Doubly interval censored and truncated data are common when assessing the survival time of bird and insect nests~\citep{heiseyABCs}.
\citet{heiseyModelling}, building on long-standing methodology~\citep{dempsterMaximum,turnbullEmpirical}, develops a likelihood-based inference framework, allowing for arbitrary truncation patterns and double interval censoring.
...
We adapt this framework for CIS, when each individual has their own visit schedule (see \cref{perf-test:sec:model}).
...
\section{Handling false negatives}
...
We first approximate the \cref{E-ATACCC} posterior estimate of the hazard as $\logit{\vec{h}} \dist \MNorm(\vec{\mu}_A, \matr{\Sigma}_A)$ where $\vec{h}$ is the hazard, and $\vec{\mu}_A$ and $\matr{\Sigma}_A$ are the mean and covariance matrix estimated using samples of $\logit{\vec{h}}$ from the previous study's posterior.
...
We simulate data under this model by, for each test in a detectable individual, calculating $\psens(t-b)$, where $t$ is the day of the test and $b$ is the day the episode began.
...
We simulate datasets under four conditions: a constant $\psens$ of 0.6, 0.8, or 1.0, or the varying test sensitivity model proposed at the end of \cref{imperf-test:sec:simulate}.
\todo{Decide where to introduce a varying test sensitivity model}
...
For each simulated dataset, we estimate the survival function using the likelihood proposed in \cref{imperf-test:sec:modelling} and assuming $\psens$ equals 0.6, 0.8, or 1.0.
If the dataset was simulated using a constant $\psens$, and that value of $\psens$ matches the value assumed during inference, we refer to $\psens$ as being correctly specified; otherwise we refer to it as misspecified.
...
Next, we considered the consequence of $\psens$ being misspecified, using the model combination prior; the better performing prior in the correctly specified case.
...
Therefore, we will apply this model to the real CIS data in the next section.
\begin{figure}
  % \makebox[\textwidth][c]{
...
In this section we apply the approach described in this chapter to the CIS episodes dataset we described in \cref{E-perf-test:sec:approach}.
...
A very weak, almost uninformative, prior on this quantity causes the posterior estimate to be much higher than the estimate from \citet{birrellRTM2}.
...
The median using $r\inform$ matched \cref{E-ATACCC}'s median estimate and is a principled choice because it is based directly on the prior work \citet{birrellRTM2}.
Therefore, we use this estimate in the application to the CIS data.
\begin{figure}
...
In this chapter, we developed a novel method to incorporate false negatives into the survival analysis of \cref{E-perf-test}.
we then applied this method to the CIS data, estimating the tail of the duration distribution without the strong model assumptions and extrapolation in \cref{E-ATACCC}.
...
Prevalence was fairly constant over the period we chose, suggesting the same is true of the incidence.
