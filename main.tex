...
Omitting the conditioning on $\vec{\theta}$ and $i_j = i$, this gives:
...
\begin{align}
  ...
  &= 1 - \sum_{i=1}^{\Ncis} \prob(O_j = \emptyset \mid i_j = i \mid \vec{\theta}) P(i_j = i \mid \vec{\theta}) \\
  &= 1 - \frac{1}{\Ncis}\sum_{i=1}^{\Ncis} \prob(O_j = \emptyset \mid i_j = i \mid \vec{\theta}) \\
  &= \frac{1}{\Ncis} \sum_{i=1}^{\Ncis} (1 - \prob(O_j = \emptyset \mid i_j = i \mid \vec{\theta}))
...
For tractability, we include only tests between the negative tests between $l_j^{(b)}-1$ and $r_j^{(e)}$. 
...
Similar likelihoods have previously appeared in the literature~\cite[e.g.][eq.\ (2)]{piresIntervalMisclassify}.
