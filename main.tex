...
\section{Modelling and inference framework} \label{sec:inference}
...
In this section, we give the generating process and then derive a statistical model for the case where that there are no misclassified test results; in \cref{sec:false-negatives} we generalise the framework to include false negatives.
...
This CIS data is unique, and is likely to serve as a template for population prevalence studies in future pandemics.
...
This is a higher proportion than estimated using other studies such as ATACCC (see \cref{imperf-test:fig:cis-estimates}), leading to a longer mean, driven by the better information resulting from the longer follow-up of the CIS.
...
Our methodology depends on a few important assumptions.
...
The simulation study assumed non-independence between infections in the same individual, with each individual experiencing at most one infection.
