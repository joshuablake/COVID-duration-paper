...
This pandemic highlighted the critical need for an effective public health response, informed by key quantities such as estimates of the \emph{incidence of infection}, \ie the rate of new infections per calendar time~\citep{freemanPrevalence}, which drives important health outcomes including hospital admissions and deaths. 
...
Once invited and consented, swabs were taken at a first visit followed by an optional 4 further weekly visits (giving a total of 5 swabs on days 0, 7, 14, 21, 28 from enrolment) after which visits were four-weekly.
...
We focus on the period between 16 October 2020 ($t=1$) and 6 December 2020 ($t=T=58$) inclusive over which the prevalence, as estimated by CIS, was approximately stable at 1\%~\citep{onsCISdec2020}.
...
We denote by $\sched_i$ the set of times (in days) individual $i \in \{ 1, \dots, \Ncis \}$ is tested.
The smallest element of $\sched_i$ is individual $i$'s last test prior to $t=1$, if it exists, or their first test following enrollment in the study otherwise. All subsequent tests, even those that occur after day $T$, are included in $\sched_i$.
