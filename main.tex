...
We focus on the period between 16 October 2020 and 6 December 2020 inclusive over which the CIS estimated stable infection prevalence of around 1\%~\citep{onsCISdec2020} and exclude void tests, assuming they are uninformative.
Stable infection prevalence means that the shared population-level force of infection changes slowly, making our independence assumption (see \cref{sec:modelling}) a reasonable approximation.
...
The results in Figure \cref{imperf-test:fig:simulation-results}(A and B) show that even a weak prior on the survival time allows recovery of the true survival time.
\begin{figure}
...
Although the CIS data are unique, this work may serve as a template for studies in future pandemics~\citep{hallettModule} as our methodological framework is generic.
The study design did not explicitly consider the need to estimate duration of positivity from it, which made this study challenging.
To address this issue, we have developed a flexible simulation framework that can assist with  more efficient survey designs; for example, embedding an intensively sampled, ATACCC-like study within a CIS-like study.
...
Previous work~\citep[e.g.][]{haySerosolver} has considered relaxing this assumption by including shared force of infection parameter(s), at the cost of a more complex likelihood and a possible need for large amounts of data imputation which would be computational prohibitive.
...
Our simulation study showed that, while a strongly informative prior for the survival time is helpful, it is not required and our method can successfully recover the truth using only a weakly informative prior.
Such a prior could be available early in a pandemic based on a small, intensive study (\eg First Few X) or expert elicitation based on similar diseases.
