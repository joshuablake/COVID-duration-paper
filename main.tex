...
We consider both a weakly and strongly informative prior for $\vec{\theta}$ (see the Web Appendix).
...
\end{align}
A full derivation is in the Web Appendix.
...
In the Web Appendix, we show that:
\begin{align}
  1 - p_u
...
By considering the case of whether this occurred or not and assuming $\psens$ is known and fixed, in the Web Appendix we show
\begin{align}
p_{ik}'
...
In the Web Appendix we show this gives:
\begin{align}
1 - p_{iu}'
...
We simulate a dataset of detected episodes that has the same characteristics as that in the CIS (see the Web Appendix for details), considering four scenarios for the test sensitivity.
...
When increasing the prior's strength, the posterior estimate moves towards the prior smoothly, as expected (see additional figure in the Web Appendix).
...
The Web Appendix referenced in \cref{sec:modelling,sec:false-negatives,sec:simulation} and the additional figure referenced in \cref{sec:CIS} and  is available with this paper at the Biometrics website on Wiley Online Library.
