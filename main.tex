...
\todo[inline]{should duration from ATACCC be mentioned here?}
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics[width=0.9\paperwidth]{figures/output/challenges}}
  \caption{%
...
Monthly testing is longer than many infection episodes; therefore, an infection episode could begin and end between tests.
...
Third, test results can be misclassified.
The sensitivity of RT-PCR tests is less than 100\%~\citep{blakeThesis}, although the specificity is very high (\ie there are a negligble number of false positives)~\citep{cisMethodsONS}.
...
Finally, to preserve privacy, the CIS data is held in the Secure Research Service (SRS)~\citep{onsSRS}, a form of Trusted Research Environment.
...
This period has fairly stable prevalence, as measured by the CIS~\citep{onsCISdec2020}.
Additionally, it is prior to either vaccination or the emergence of the alpha variant, which likely impact the duration of RT-PCR positivity~\citep{hakkiOnset,russellWithinhost}.
...
Therefore, $p_{ik}$ takes the standard form of the likelihood for doubly interval censored data without truncation:
\begin{align}
p_{ik}
...
\todo[inline]{What, if any, of the rest of this subsection should stay in the main text?}
...
\begin{figure}
  \vspace{-1.5cm}
  \makebox[\textwidth][c]{\includegraphics{figures/output/CIS_vary}}
  \caption{%
...
\todo[inline]{There's a lot of discussion points here, I've just copied these across from my thesis. I think probably too many, especially the focus on limitations; this section is about twice as long as the standard discussion section in a JASA applications article. I'm not sure which are the interesting ones to keep and which to cut though.}
